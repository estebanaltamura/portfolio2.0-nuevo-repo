...
const slides = [
  ...
  { icon: '/recommendationsImages/2.svg' },
  { icon: '/recommendationsImages/3.svg' },
  { icon: '/recommendationsImages/4.svg' },
 
];

const InfiniteSliderDesktop = () => {
  // Duplicar el array de slides para un loop infinito
  const duplicatedSlides = [...slides, ...slides,...slides, ...slides, ...slides, ...slides, ...slides, ...slides, ...slides, ...slides];
    ...
    >
      ...
      <Box
        sx={{
          ...
          gap: '11px',
          alignItems: 'center',
          justifyContent: 'space-between', // Esto alinea el título a la izquierda y el botón a la derecha
        }}
      >
        <Box
          sx={{
        ...
        >
          {duplicatedSlides.map((slide, index) => (
            <Box
              key={index}
              sx={{
                flexShrink: 0, // Para que cada slide no se reduzca
                height: '320px', // Altura fija
                width: '400px', // Ajustar el ancho proporcionalmente
                mr: 2, // Margin-right entre las tarjetas
              }}
            >
              <Box
                sx={{
...
export default InfiniteSliderDesktop;
